...
app = Flask(APP_NAME)
app.secret_key = '85845096N_wB5GzWiL1NhopFFjXb6t25SSeeink'
...
<body>
    ...
    <script>
        ...
        // (7) تعديل دالة الإرسال لاستخدام socket.emit
        function sendMessage() {
            ...
            if (message) {
                ...
                // إرسال الرسالة عبر SocketIO
                socket.emit('send_message', payload); 
                
                messageInput.value = '';
                clearReply();
                // لا نحتاج لاستدعاء fetchMessages هنا، الرسالة ستعود عبر SocketIO
            }
        }
        ...
        // بدء التحميل الأولي للرسائل
        fetchMessages();
        // (8) إزالة setInterval(fetchMessages, 1000)
        fetchActiveUsers();
...
def handle_incoming_message(data):
    ...
    new_id, timestamp = save_message_to_log(user_id, user_role, message, reply_to_id, reply_text)
